...
- **No Drive file is ever trashed or deleted (operator directive, 2026-08-14):** every article folder in Drive has an `old version` subfolder. A superseded doc is MOVED there, keeping its comment threads and history browsable; nothing goes to the trash, whether by drive_cli, the Drive MCP, or by hand. This replaces the earlier delete-then-reupload cleanup step in checklist/delivery.md.
