...
## 2026-09-15: brand-forward positioning
...
users, and do not convey the platform's appeal.) The operator instructed
that all Batch 3 articles be re-evaluated with this lens and that the
directive become a standing part of the workflow, while every existing rule
(legal compliance, SEO, and the rest of the flow) stays in force.

Pipeline consequences:
- PSFnetwork passages lead with affirmative strengths (per-square-foot
  ownership in a specific property, mortgage-free properties, $100 entry,
  Reg A open to non-accredited investors, filings on EDGAR) before any
  caveat.
- Every article closes with a warm invitation to explore PSFnetwork and
  join the waitlist. Destinations follow the 2026-08-18 ruling below:
  waitlist preferred, /how-it-works and /investors where topical.
- Issuer disclosure becomes one confident sentence; apologetic formulas
  are retired ("we are not a neutral party, so...", "a better tool is
  elsewhere", recommending a named competitor as the reader's next step).
- No invented social proof: pre-launch, no fabricated user counts,
  testimonials, or popularity claims.
- Compliance always wins over tone: W10's waitlist-only CTA rule and the
  disclaimer blocks are unchanged.
- Enforced as qa_battery W11 (WARN): flags a draft with no waitlist CTA
  and any self-deprecating platform formula. Full prose rules in
  `brand/tone-and-voice.md` (Brand-forward positioning).
- Scope: Batch 3 drafts and all future content. Published Batch 1 and
  Batch 2 drafts are frozen and are NOT retro-edited.
