# Stage 3 Review - SEC

DOMAIN FINDINGS:

Platform structural classification is accurate: Reg A LLC for Fundrise/Ark7/Arrived/Realbricks/HappyNest/psfnetwork; tokenized for Lofty; Reg A/D mixed for Mogul. The Lofty caveats (Algorand, custodial wallet, regulatory ambiguity) match what the SEC has said about tokenized securities generally. The Mogul "performance claims platform-reported, unaudited" framing is correct.

Sources cite SEC EDGAR for filing verification — best practice.

FLAGGED ISSUES:

1. **Mogul "$250 minimum at the time of writing"** - SEVERITY: LOW - FIX: Currently captioned with "as of writing" - acceptable. Watch for staleness on future revisions.

2. **"Reg A (Tier 2 allows non-accredited investors with disclosure, capped at $75 million per 12 months)"** - SEVERITY: pass - Accurate.

3. **"performance claims are platform-reported and unaudited" framing for Mogul** - SEVERITY: pass - This is the right disclosure.

VERDICT: APPROVE
