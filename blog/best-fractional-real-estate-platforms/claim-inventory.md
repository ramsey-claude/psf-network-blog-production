# Claim Inventory - best-fractional-real-estate-platforms

## Platform minimum claims (cross-ref hub + blog 4)
| # | Claim | Source target |
|---|-------|---------------|
| C1 | Ark7 $20 min | cross-ref hub E11 |
| C2 | Arrived $100 min | cross-ref blog 4 E1 |
| C3 | Realbricks $100 min | cross-ref blog 4 E2 |
| C4 | Fundrise $10 min | cross-ref blog 4 E4 |
| C5 | Lofty $50 min | cross-ref blog 4 E5 |
| C6 | HappyNest $10 min | cross-ref blog 4 E6 |
| C7 | Mogul $250 min | platform marketing (medium confidence) |
| C8 | Groundfloor varies | platform marketing |
| C9 | psfnetwork $100 entry | brand self-reported |

## Regulatory claims (cross-ref hub)
| # | Claim | Source |
|---|-------|--------|
| C10 | All US-regulated platforms file under Reg A or Reg D | sec.gov / investor.gov |
| C11 | LLC pass-through structure with K-1 reporting | irs.gov |

## Structural / methodology claims
| # | Claim | Source |
|---|-------|--------|
| C12 | 4-9% typical platform-reported yield range | cross-ref hub E12 |
| C13 | Lofty uses tokenized model on Algorand (different from Reg A LLC) | platform; flagged in blog 4 |
| C14 | Arrived focuses on single-family rentals and short-term rentals | platform marketing |
| C15 | Ark7 monthly income, secondary market after 1-year hold | platform marketing |

## Dropped / softened
| # | Original element | Decision | Reason |
|---|------------------|----------|--------|
| D1 | "$337M Largest AUM (Arrived)" stat card | Replaced: "varies by platform; Arrived publishes AUM in offering circular" | Specific $337M figure cannot be sourced without an audit; verify-per-offering language safer |
| D2 | "18.8% IRR (Mogul, unaudited)" stat card | Dropped | Specific IRR figure is platform-promotional; we cite caveat in body instead |
| D3 | "Best for international investors" section | Reframed to "Best for property-specific exposure" | US-only scope; do not market international |
| D4 | "psfnetwork's international/sq-ft angle" framing | Rewrite as "square-foot model" only | Same |

Total: 15. Dropped/softened: 4.
